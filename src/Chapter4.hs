...
instance Functor List where
  ...
  fmap _ Empty = Empty
  fmap f (Cons a remaining) = Cons (f a) (fmap f remaining)
...
concatList Empty l2 = l2
concatList l1 Empty = l1
...
instance Applicative List where
  ...
  (<*>) Empty _ = Empty
  (<*>) _ Empty = Empty
  (<*>) (Cons f rest) l = concatList (fmap f l) (rest <*> l)
...
-- andM m1 m2 = pure (&&) <*> m1 <*> m2 -- (Without >>= and anonymous function)
...
treeToList Leaf = []
treeToList (Node e (l,r)) = (treeToList l) ++ [e] ++ (treeToList r)
